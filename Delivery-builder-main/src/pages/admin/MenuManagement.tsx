...
const TypeDialog = ({ open, setOpen, editingItem, onSave, pizzaExtras }) => {
  ...
  useEffect(() => {
    if (open) {
      if (editingItem) {
        ...
        setSelectedExtraIds(
          new Set(editingItem.availableExtras?.map((e) => e.id) || []),
        );
      } else {
  ...
  const handleExtraChange = (extraId: string, checked: boolean) => {
    setSelectedExtraIds((prev) => {
      ...
      if (checked) newSet.add(extraId);
      else newSet.delete(extraId);
      return newSet;
    });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {editingItem ? "Editar Tipo de Pizza" : "Novo Tipo de Pizza"}
          </DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <Label htmlFor="typeImage">Imagem do Tipo</Label>
            <Input
              id="typeImage"
              type="file"
              accept="image/*"
          ...
          <div>
            <Label>Adicionais Disponíveis para este Tipo</Label>
            <Card className="p-3 mt-2 max-h-48 overflow-y-auto">
              <div className="space-y-2">
                {pizzaExtras.map((extra) => (
                  <div key={extra.id} className="flex items-center space-x-2">
                    <Checkbox
                      id={`type-extra-${extra.id}`}
                      checked={selectedExtraIds.has(extra.id)}
                      onCheckedChange={(checked) =>
                        handleExtraChange(extra.id, !!checked)
                      }
                    />
                    <Label
                      htmlFor={`type-extra-${extra.id}`}
  ...
  useEffect(() => {
    if (open) {
      if (editingItem) {
        ...
        const associatedTypeIds = pizzaTypes
          .filter((type) =>
            type.availableExtras?.some((extra) => extra.id === editingItem.id),
          )
          .map((type) => type.id);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {editingItem ? "Editar Adicional" : "Novo Adicional"}
          </DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
      ...
      <AdminLayout>
        <div className="space-y-6">
          ...
          <FlavorDialog
            ...
            setOpen={setIsFlavorDialogOpen}
            editingItem={editingFlavor}
            onSave={handleSave}
            pizzaTypes={pizzaTypes}
          />
          <ExtraDialog
            open={isExtraDialogOpen}
            setOpen={setIsExtraDialogOpen}
            editingItem={editingExtra}
            onSave={handleSave}
            pizzaTypes={pizzaTypes}
          />
        </div>
