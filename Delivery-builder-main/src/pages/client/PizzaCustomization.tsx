...
export const PizzaCustomization = () => {
  ...
  const handleFlavorSelection = (flavor: PizzaFlavor) => {
    if (isHalfAndHalf) {
      ...
      if (isSelected) {
        ...
        if (activePlacement === flavor.id) setActivePlacement('whole');
  ...
  const handleExtraToggle = (extraId: string) => {
    setExtraSelections(prevSelections => {
        ...
        const isCurrentlySelected = newSelections.some(s => s.extraId === extraId && s.placement === activePlacement);
        if (isCurrentlySelected) {
            return newSelections.filter(s => !(s.extraId === extraId && s.placement === activePlacement));
        }
  ...
  // --- FUNÇÃO DE CÁLCULO DE PREÇO ATUALIZADA ---
  const calculateTotalPrice = () => {
    if (!pizzaType || selectedFlavors.length === 0) return 0;

    const flavorPrice = selectedFlavors.reduce((sum, flavor) => {
    ...
    return (pizzaType.basePrice + flavorPrice + extrasPrice + crustPrice) * quantity;
  };

  const handleAddToCart = () => {
    if (selectedFlavors.length === 0 || !pizzaType) return;
    const appliedExtras: AppliedExtra[] = extraSelections.map(selection => ({
      extra: extras.find(e => e.id === selection.extraId)!,
      onFlavor: selection.placement === 'whole' ? null : flavors.find(f => f.id === selection.placement) || null,
    }));
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
              <Card>
                <CardHeader><div className="flex justify-between items-center"><CardTitle>1. Escolha o(s) Sabor(es)</CardTitle><div className="flex items-center space-x-2"><Label htmlFor="half-half-switch">Dois Sabores</Label><Switch id="half-half-switch" checked={isHalfAndHalf} onCheckedChange={(checked) => {setIsHalfAndHalf(checked); setSelectedFlavors([]); setExtraSelections([]);}} /></div></div><p className="text-sm text-muted-foreground pt-1">{isHalfAndHalf ? "Selecione até dois sabores." : "Selecione um sabor."}</p></CardHeader>
                <CardContent><div className="grid gap-3">{flavors.map((flavor) => {const isSelected = selectedFlavors.some(f => f.id === flavor.id); return (<div key={flavor.id} className={`p-4 border rounded-lg cursor-pointer transition-colors flex items-start gap-4 ${isSelected ? "border-red-500 bg-red-50" : "border-gray-200 hover:border-gray-300"}`} onClick={() => handleFlavorSelection(flavor)}>{isHalfAndHalf && <Checkbox checked={isSelected} className="mt-1" />} {flavor.imageUrl ? <img src={`http://localhost:8090${flavor.imageUrl}`} alt={flavor.name} className="w-20 h-20 rounded-md object-cover flex-shrink-0" /> : <div className="w-20 h-20 rounded-md bg-gray-100 flex items-center justify-center text-gray-400 flex-shrink-0"><Pizza className="h-8 w-8" /></div>} <div className="flex-1 flex justify-between items-start"><div><h3 className="font-semibold text-gray-900">{flavor.name}</h3><p className="text-sm text-gray-600 mt-1">{flavor.description}</p></div><Badge variant="secondary" className="flex-shrink-0 ml-2">{flavor.price > 0 ? `+${formatPrice(flavor.price)}` : 'Incluso'}</Badge></div></div>);})}</div></CardContent>
              </Card>
              <Card>
                <CardHeader><CardTitle>2. Adicionais (Opcional)</CardTitle></CardHeader>
                <CardContent className="space-y-4">{isHalfAndHalf && selectedFlavors.length === 2 && (<div className="p-3 bg-gray-50 rounded-lg"><Label className="font-semibold">Escolha onde adicionar:</Label><ToggleGroup type="single" value={activePlacement} onValueChange={(value) => value && setActivePlacement(value)} className="grid grid-cols-3 gap-2 mt-2"><ToggleGroupItem value={selectedFlavors[0].id} aria-label={`Metade ${selectedFlavors[0].name}`}>Metade {selectedFlavors[0].name}</ToggleGroupItem><ToggleGroupItem value="whole" aria-label="Pizza Toda">Pizza Toda</ToggleGroupItem><ToggleGroupItem value={selectedFlavors[1].id} aria-label={`Metade ${selectedFlavors[1].name}`}>Metade {selectedFlavors[1].name}</ToggleGroupItem></ToggleGroup></div>)}{extras.map((extra) => {const isChecked = extraSelections.some(sel => sel.extraId === extra.id && sel.placement === activePlacement); const placements = extraSelections.filter(sel => sel.extraId === extra.id); return (<div key={extra.id} className="p-3 border rounded-lg"><div className="flex items-start gap-3"><Checkbox id={extra.id} checked={isChecked} onCheckedChange={() => handleExtraToggle(extra.id)} className="mt-1"/><div className="flex-1"><Label htmlFor={extra.id} className="font-medium cursor-pointer">{extra.name} <Badge variant="outline" className="ml-2">+{formatPrice(extra.price)}</Badge></Label><p className="text-sm text-muted-foreground">{extra.description}</p>{placements.length > 0 && (<div className="flex flex-wrap gap-1 mt-2">{placements.map(p => {const flavor = flavors.find(f => f.id === p.placement); return <Badge key={p.placement} variant="secondary" className="bg-green-100 text-green-800"><CheckCircle className="h-3 w-3 mr-1"/>{flavor ? `Metade ${flavor.name}` : 'Pizza Toda'}</Badge>})}</div>)}</div></div></div>);})}</CardContent>
              </Card>
            ...
            <div className="lg:col-span-1">
              <Card className="sticky top-8">
                ...
                <CardContent className="space-y-4">
                  {/* --- SEÇÃO DE RESUMO DO PEDIDO ATUALIZADA --- */}
                  <div><h3 className="font-semibold text-gray-900">{pizzaType?.name}</h3><p className="text-sm text-gray-600">Sabor: {selectedFlavors.map(f => f.name).join(' / ')}</p></div>
                  <div className="space-y-1 text-sm">
                      <h4 className="font-medium text-gray-900 mb-1">Detalhes do Valor:</h4>
                      <div className="flex justify-between"><span className="text-gray-600">Base</span><span>{formatPrice(pizzaType?.basePrice || 0)}</span></div>
                      {selectedFlavors.map(flavor => (
                        <div key={flavor.id} className="flex justify-between">
                            <span className="text-gray-600">↳ Sabor: {flavor.name}{isHalfAndHalf && ' (Metade)'}</span>
                            <span className="text-gray-900">+ {formatPrice(isHalfAndHalf ? flavor.price / 2 : flavor.price)}</span>
                        </div>
                      ))}
                      {selectedCrust && selectedCrust.price > 0 && <div className="flex justify-between"><span className="text-gray-600">↳ Borda: {selectedCrust.name}</span><span className="text-gray-900">+ {formatPrice(selectedCrust.price)}</span></div>}
                      {extraSelections.map(sel => {
                          const extra = extras.find(e => e.id === sel.extraId);
                          const placementFlavor = flavors.find(f => f.id === sel.placement);
                          const placementText = placementFlavor ? `(Metade ${placementFlavor.name})` : '(Pizza Toda)';
                          return <div key={`${sel.extraId}-${sel.placement}`} className="flex justify-between"><span className="text-gray-600">↳ Adicional: {extra?.name} {isHalfAndHalf && placementText}</span><span className="text-gray-900">+ {formatPrice(extra?.price || 0)}</span></div>
                      })}
                  </div>
                  {/* --- FIM DA SEÇÃO ATUALIZADA --- */}
                  <div><h4 className="font-medium text-gray-900 mb-2 mt-4">Quantidade:</h4><div className="flex items-center space-x-3"><Button variant="outline" size="sm" onClick={() => setQuantity(Math.max(1, quantity - 1))} disabled={quantity <= 1}><Minus className="h-4 w-4" /></Button><span className="font-semibold w-8 text-center">{quantity}</span><Button variant="outline" size="sm" onClick={() => setQuantity(quantity + 1)}><Plus className="h-4 w-4" /></Button></div></div>
